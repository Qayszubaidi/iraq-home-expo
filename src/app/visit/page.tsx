...
<section className="contentSplit"><div><span className="eyebrow dark">The Venue</span><h2>{venue.name}</h2></div><div>{venue.description.map((p,i)=><p key={i}>{p}</p>)}</div></section>
...
<section className="audienceSection"><div><span className="eyebrow dark">Professional Visitors</span><h2>Who should visit?</h2><p>The exhibition is designed for professionals involved in buying, sourcing, specifying, distributing and promoting products for the Iraqi market.</p></div><div className="audienceGrid">{visitorProfile.map((x,i)=><div key={x.title}><span>{String(i+1).padStart(2,"0")}</span><strong>{x.title}</strong><p>{x.description}</p></div>)}</div></section>
