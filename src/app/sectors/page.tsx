...
<section className="sectorIndex">{displaySectors.map((s,i)=><Link href={`/sectors/${s.slug}`} key={s.slug} className="sectorIndexCard"><Image src={s.image} fill alt={`${s.title} exhibition sector at Iraq Home Expo 2027 in Baghdad`}/><div className="cardShade"/><span>0{i+1}</span><div><h2>{s.title}</h2><p>{s.short}</p><b>Explore sector ↗</b></div></Link>)}</section></>}